...
var check = exports.check = function (value, pattern) {
  ...
  if (result) {
    var err = new _CheckError["default"](result.message);
...
var Match = exports.Match = {
  ...
  /*arguments*/
  {
    return new _OneOf(_underscore["default"].toArray(arguments));
  },
  Any: ['__any__'],
...
var _OneOf = function _OneOf(choices) {
  if (_underscore["default"].isEmpty(choices)) throw new _CheckError["default"]("Must provide at least one choice to Match.OneOf");
...
var testSubtree = function testSubtree(value, pattern) {
  ...
  if (pattern instanceof Array) {
    ...
    if (!_underscore["default"].isArray(value) && !_underscore["default"].isArguments(value)) {
      return {
  ...
  _underscore["default"].each(pattern, function (subPattern, key) {
  ...
  var allKeys = function allKeys(obj) {
    ...
    if (_underscore["default"].isObject(obj)) {
      for (var key in obj) {
        keys.push(key);
      }
    }
  ...
  for (var keys = allKeys(value), i = 0, length = keys.length; i < length; i++) {
    ...
    if (_underscore["default"].has(requiredPatterns, key)) {
    ...
    } else if (_underscore["default"].has(optionalPatterns, key)) {
  ...
  var keys = _underscore["default"].keys(requiredPatterns);
...
var ArgumentChecker = function ArgumentChecker(args, description) {
  var self = this; // Make a SHALLOW copy of the arguments. (We'll be doing identity checks
  // against its contents.)

  self.args = _underscore["default"].clone(args); // Since the common case will be to check arguments in order, and we splice
...
_underscore["default"].extend(ArgumentChecker.prototype, {
  checking: function checking(value) {
    var self = this;
    if (self._checkingOneValue(value)) return; // Allow check(arguments, [String]) or check(arguments.slice(1), [String])
    // or check([foo, bar], [String]) to count... but only if value wasn't
    // itself an argument.

    if (_underscore["default"].isArray(value) || _underscore["default"].isArguments(value)) {
      _underscore["default"].each(value, _underscore["default"].bind(self._checkingOneValue, self));
    }
  },
  _checkingOneValue: function _checkingOneValue(value) {
    ...
    for (var i = 0; i < self.args.length; ++i) {
      ...
      if (value === self.args[i] || _underscore["default"].isNaN(value) && _underscore["default"].isNaN(self.args[i])) {
  ...
  throwUnlessAllArgumentsHaveBeenChecked: function throwUnlessAllArgumentsHaveBeenChecked() {
    var self = this;
    if (!_underscore["default"].isEmpty(self.args)) throw new _CheckError["default"]("Did not check() all arguments during " + self.description);
  }
});
...
var _prependPath = function _prependPath(key, base) {
  if (typeof key === "number" || key.match(/^[0-9]+$/)) key = "[" + key + "]";else if (!key.match(/^[a-z_$][0-9a-z_$]*$/i) || _underscore["default"].contains(_jsKeywords, key)) key = JSON.stringify([key]);
